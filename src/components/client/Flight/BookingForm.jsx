...
const BookingForm = ({
  ...
  const shareUrl = useMemo(() => {
    ...
    return window.location?.href || (`${window.location.origin}/flight/booking/details?${searchParams?.toString?.() || ''}`);
  }, [searchParams]);
  ...
  return (
    <div className="justify-center px-2 sm:px-4">
      ...
      <div className="text-center mb-4">
        <span className="text-sm text-muted text-center">All flight times displayed are local</span>
      </div>
      ...
      <div className="checkout-btn mb-3">
        <button
          type="button"
          onClick={() => handlePayment(formData.payment_method)}
          disabled={!isFormValid || !formData.payment_method || isProcessing}
          className="btn bg-[#0ea5e9] w-100 text-white font-semibold py-3 px-4 rounded-5 hover:bg-[#1982FF] transition duration-200 disabled:bg-gray-400 disabled:cursor-not-allowed"
        >
          {isProcessing ? (
      ...
      <p className="mt-2 text-sm text-muted px-4 lh-2 mb-2">
        By clicking the 'Book now' button above you confirm you have read and accepted the <b>Terms and Conditions</b> and the <b>Fare Rules</b> of your booking
      </p>
