...
import { getAirlineLogo, getAirlineName } from '../../utils/utils';
...
const FlightSegment = ({ flight, segmentType, formatDate, formatTime, calculateDuration, getAirportName }) => {
  ...
  const middleSegments = flight.segments.slice(1, -1); // removes first and last

  return (
<div className="mb-4 rounded-3 bg-white">
    <div key="" className="row g-3 mb-2 pb-2 border-bottom">
      <div className="col-md-12">
        <div className="d-flex flex-column mb-3 flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
          ...
          <div className="text-start w-100 w-md-25">
            <div className="fw-semibold text-dark">{formatDate(firstSegment.departureDate)}</div>
            <div className="text-muted small">Departure Time: <b>{formatTime(firstSegment.departureDate)}</b></div>
            <div className="text-muted small">From: <b>{getAirportName(firstSegment.departure)}</b></div>
          </div>
          ...
          <div className="text-end w-100 w-md-25">
            <div className="fw-semibold text-dark">{formatDate(lastSegment.arrivalDate)}</div>
            <div className="text-muted small">Arrival Time: <b>{formatTime(lastSegment.arrivalDate)}</b></div>
            <div className="text-muted small">To: <b>{getAirportName(lastSegment.arrival)}</b></div>
          </div>
        </div>

        {/* Segments */}
        {flight.segments.map((segment, index) => (
          <div key={index} className="border rounded-4 p-2 mb-2">
              ...
              <div className="d-flex justify-content-start text-start align-items-center gap-2">
                ...
                <div className="pb-2 border-md border-end w-100 pe-5">
                  <span className="fw-bold text-sm">
                      {getAirlineName(segment.airline) || 'N/A'}
                  </span>
                  <span className="text-secondary d-block fw-light mt-1 lh-0">
                    <small>
                      Flight No. {segment.flightNum} - {segment.departure} - {segment.arrival}
                    </small>
                  </span>
                </div>
              </div>
            ...
            <div key={index} 
              ...
              <div className="flex justify-between gap-4 w-full mx-auto mt-2">
                ...
                <div className="text-center">
                  <div className="text-md font-semibold text-gray-800">{formatDate(segment.departureDate)} {formatTime(segment.departureDate)}</div>
                ...
                {/* Arrival */}
                <div className="text-center">
                  <div className="text-md font-semibold text-gray-800">{formatDate(segment.arrivalDate)} {formatTime(segment.arrivalDate)}</div>
                  <div className="text-xs text-gray-500">{getAirportName(segment.arrival)}</div>
                </div>
              </div>
              {/* Tags */}
              <div className="flight--tco small--text d-flex flex-wrap mt-2 mb-2 text-muted fw-bold gap-2" style={{ fontSize: '12px' }}>
